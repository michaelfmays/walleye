...
source("helper_functions.R") # Various functions created for use in this project
source("data.R") # All of the data reading/cleaning/etc

# Just some ggplot theme options I tend to like
...
```{r data.R, eval=FALSE, include=T, echo=T, message=T}
...
The second YOY model fit was a negative binomial generalized linear model model. Since the response is count data, Poisson regression was an obvious first choice. 
```{r poi YOY, echo=T, message=F, warning=F, include=T}
# Poisson
gpoi <- glm(yoy_catch~
...
However, the data had significant overdispersion, even when accounting for lake-specific effects via a Poisson GLMM; this is shown in the next section, which covers diagnostics. Given overdispersion, an obvious second choice was a negative binomial GLMM; the Poisson-Gamma mixture offers an additional dispersion parameter to account for the variance unexplained by the random effect. However, this model fell into the opposite problem: underdispersion due to very nearly observation-level random effects (a nice description of this problem is given by Bolker [-@Stack]). As with the Poisson case, this is shown in the next section. In addition, very few lakes in the dataset were sampled in more than one year, and given this particular set of data, the resulting negative binomial GLMM over-fit to an unidentifiable degree. 

```{r NB lake RE YOY, cache=T, echo=T, message=F, include=T}
## Negative binomial model - mixed effects
# Fit initial model using a quick (but less accurate) algorithm
#   to use theta estimate
...
# Fit actual model using theta estimate as starting value
#   (plus more algorithm iterations) to help model convergence
g1 <- glmer(yoy_catch~
            offset(log_km_surv)+
            log_size_acre+
            I(s_year)+
            ...
            (1|wbic),
    data=fish_sc, 
    # Use MASS parameterization for consistency
...
```{r, echo=F, warning=T, message=F, include=T}
# Results in degenerate Hessian w/ 1 negative eigenvalue
glmer(yoy_catch~
            offset(log_km_surv)+
            log_size_acre+
            I(s_year)+
...
Instead, the lake-specific effects from a mixed-effects model were proxied by the limnological characteristics of each lake, which begs the question of model selection.

```{r NB FE YOY, cache=T, echo=T, message=F, include=T}
## Negative binomial model - fixed effects
# Fit initial model using a quick (but less accurate) algorithm
#   to use theta estimate
...
The initial model used to assess juvenile walleye recruitment was identical to the negative binomial fixed effect detailed above with the response $Y^{YOY}$ changed its JUV equivalent $Y^{JUV}$.

```{r NB lake RE JUV, cache=T, echo=T, message=F, include=T}
## Negative binomial model - mixed effects
# Fit initial model using a quick (but less accurate) algorithm
#   to use theta estimate
...
```{r, echo=T, include=T}
# The random effect has variance 0 because all random intercepts are 0
#   This suggests an unidentifiable variance-covariance matrix
sum(ranef(g1j)$wbic$`(Intercept)`!=0)
```

However, unlike the YOY model, the JUV model suffered from zero inflation, meaning the data contains substantially more zero-valued responses than the model predicts. Like the dispersion mentioned above, a plot of this zero-inflation is shown in the next section on diagnostics. Zero-inflation results in poor overall model fit, potentially biases estimators, and leads to underestimated standard errors. 

Instead, I fit a zero-inflated negative binomial mixed-effects model.
...
```{r prob-stuff, eval=F, echo=F, message=F, include=F}
...
It is worth setting out the motivation for testing the accuracy of the models using receiver operating characteristic (ROC) curves. Simply put, even though the dataset contains a balanced successful/unsuccessful recruitment ratio, it is likely that the ratio is unbalanced across all lakes. This is even more likely when accounting for the presence of unreliable surveys, which were removed from the dataset for this analysis. Mere accuracy is not enough for a model to be worth considering when successes and failures are unbalanced in the population; given a different dataset with proportion of successful recruitments $p$, naively guessing that a recruitment was successful (if $p>0.5$) or unsuccessful (if $p<0.5$) will yield better-than-random accuracy. 

ROC curves plot the cumulative true positive rate ("sensitivity") against the cumulative true negative rate ("specificity") for the model, and the resulting curve can be thought of as the model's power as a function of its type I error. That is, ROC curves measure the probability of detecting true positives with respect to the probability of detecting false positives. The area under the ROC curve (AUC) corresponds to the probability that the model will assign a random true positive response a predicted probability of being a success higher than a random true negative response, reflecting how well the model discriminates between the two. The table below more explicitly shows what ROC curves measure.
...
A series of 1000 ROC curves were used to assess the model's discriminatory capacities. For each iteration, the data was randomly divided into training and testing sets with a 70/30 split. The model was fit on the training dataset and then predicted the probability of successful recruitment on both the training and testing data sets (separately). Then, ROC curves were fit for the model's predictions on each dataset, resulting in two ROC curves.
...
In order for the models to meaningfully improve on random guessing, they have to out-perform the baseline true positive rate, which happened to be exactly 0.5 in the full dataset. In other words, the models are worth considering if they perform measurably better than a coin flip. 

```{r roc, eval=F, cache=T, echo=T, message=F, include=T}
...
These ROC curves are shown below.

```{r roc-plot, echo=T, message=F, warning=F, fig.cap="ROC Curve on Training and Test Datasets"}
...
```{r ecdf, echo=T, message=F, warning=F, fig.height=7, fig.cap="Top: Distribution of Mann--Whitney U Test P-Values by Dataset\newline Bottom: Empirical and Theoretical Cumulative Distribution Functions for Mann--Whitney U Test P-Values"}
